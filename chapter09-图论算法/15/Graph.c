...
CostType Kruskal1(Graph G, Edge IgnoredE)
{
    int i, EdgesAccepted, NumSameForest;
    DisjSet S;
    PriorityQueue H;
    CostType TotalCost;
    SetType Uset, Vset;
    Edge E, EdgeRecord[G->Size - 1];
    ...
    if (IgnoredE == NULL) {
        ...
        for (i = 0; i < G->Size - 1; i++)
            if (Kruskal1(G, EdgeRecord[i]) == TotalCost)
                NumSameForest++;
        printf("Number of Forest with same total cost: %d\n", NumSameForest);
    }
    return TotalCost;
}

CostType Kruskal(Graph G)
{
    return Kruskal1(G, NULL);
}
